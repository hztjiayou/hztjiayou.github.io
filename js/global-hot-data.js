$(document).ready(setTimeout(function () { // 延迟1s执行，保证其余的先加载
        ...
        if (COMMENT_COOKIE == '' || new Date().getTime() - COMMENT["date"] > 60 * 1000 * 10) { // request per 10 minutes
            ...
            console.log("request url:" + "https://api.github.com/repos/hztjiayou/blog_comment/issues/comments?sort=created&direction=desc&per_page=10&page=1");
            $.getJSON("https://api.github.com/repos/hztjiayou/blog_comment/issues/comments?sort=created&direction=desc&per_page=10&page=1", function (result) {
        ...
        if ($("#index_hot_div").length > 0) {
            ...
            $.ajaxSettings.async = false;
            console.log("request url:" + "https://api.github.com/repos/hztjiayou/blog_comment/issues?per_page=10&sort=comments");
            $.getJSON("https://api.github.com/repos/hztjiayou/blog_comment/issues?per_page=10&sort=comments", function (result) {
                $.each(result, function (i, item) {
                    // 标签配色
                    if (i % 4 == 0) {
